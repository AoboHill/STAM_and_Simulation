...
integration_simulationT <- function(parameter.list,nbins,M){
  ...
  prob_spike0 <- function(alpha,beta,nConstant = 1){
    ...
    return((1/nConstant) * pbeta(1/(M*nConstant),alpha,beta))
  ...
  prob_spike1 <- function(alpha,beta,nConstant = 1){
    ...
    return((1/nConstant) * (1-pbeta(1-1/(M*nConstant),alpha,beta)))
...
##### equal size 
d = data.frame(x=rep(seq(0.5,50,0.5),99), 
               y=rep(seq(2,100,1),each=100), 
               z=c(t(heat.map1)))
levelplot(z~x*y,data=d,col.regions=colorRampPalette(c("yellow","orange","red", "black")),at=c(seq(0,0.8,length=30)),xlab="Time",ylab = "Number of Bins",main="Errors by using Uniformly Spaced Bin Points")
...
d = data.frame(x=rep(seq(0.5,50,0.5),99), 
               y=rep(seq(2,100,1),each=100), 
               z=c(t(heat.map2)))
levelplot(z~x*y,data=d,col.regions=colorRampPalette(c("yellow","orange","red", "black")),at=c(seq(0,0.8,length=20)),xlab="Time",ylab = "Number of Bins",main="Errors by using Chebyshev-Lobatto Points")


########### chebyshev one
levelplot(z~x*y,data=d,col.regions=colorRampPalette(c("blue","light blue","light green", "yellow","orange","red","black")),at=c(seq(0,0.8,length=400)),xlab="Time",ylab = "Number of Bins",main="Errors Introduced by Using Uniformly Spaced Bin Points")
levelplot(z~x*y,data=d,col.regions=colorRampPalette(c("blue","light blue","light green", "yellow","orange","red","black")),at=c(seq(0,0.8,length=400)),xlab="Time",ylab = "Number of Bins",main="Errors Introduced by Using Chebyshev-Lobatto Points")
